...
public:
	...
	void render( Chunk *chunk, int x, int y, int z, int8_t direction, std::vector<Vertex> &vertex_buffer )
	{
		...
		if ( direction == -1 ) // Negative X direction
		{
			...
			vertex_buffer.push_back( Vertex( x, top, z, 0, left_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x, top, z, 0, left_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x, y, z + 1, 0, right_u, top_v ) );
			vertex_buffer.push_back( Vertex( x, top, z + 1, 0, right_u, bottom_v ) );
		}
		else if ( direction == 1 ) // Positive X direction
		{
			vertex_buffer.push_back( Vertex( x + 1, y, z, 0, left_u, top_v ) );
			vertex_buffer.push_back( Vertex( x + 1, top, z, 0, left_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x + 1, y, z + 1, 0, right_u, top_v ) );
			vertex_buffer.push_back( Vertex( x + 1, top, z, 0, left_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x + 1, top, z + 1, 0, right_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x + 1, y, z + 1, 0, right_u, top_v ) );
		}
		...
		{
			vertex_buffer.push_back( Vertex( x, top, z, 0, left_u, top_v ) );
			vertex_buffer.push_back( Vertex( x, top, z + 1, 0, left_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x + 1, top, z, 0, right_u, top_v ) );
			vertex_buffer.push_back( Vertex( x + 1, top, z, 0, right_u, top_v ) );
			vertex_buffer.push_back( Vertex( x, top, z + 1, 0, left_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x + 1, top, z + 1, 0, right_u, bottom_v ) );
		}
		else if ( direction == -3 ) // Negative Z direction
		{
			...
			vertex_buffer.push_back( Vertex( x + 1, y, z, 0, right_u, top_v ) );
			vertex_buffer.push_back( Vertex( x, top, z, 0, left_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x + 1, top, z, 0, right_u, bottom_v ) );
			vertex_buffer.push_back( Vertex( x + 1, y, z, 0, right_u, top_v ) );
		}
